...
function makeWrapLine(str, newlineIndent) {
    var ret = '',
...
module.exports = {
    help: function(options) {
        ...
        if(options.usage) {
            ...
            for(var i = 0; i < len; i++) {
                ret += makeWrapLine((i === 0 ? colors.bold(usageString) : indent) + colors.green(packageName) + ' ' + usage[i], indent.length + 4) + '\n';
            }
        }
                ...
                if(sections.hasOwnProperty(section)) {
                    ...
                    if(entry.options) {
                        ...
                        for(optName in entry.options) { // Now do the processing
                            if(entry.options.hasOwnProperty(optName)) {
                                ret += makeWrapLine('    ' + colors.yellow(optName) + (new Array(maxNameLength - optName.length + 3).join(' ')) + entry.options[optName], maxNameLength + 6) + '\n';
                            }
                        }
                    }
                }
